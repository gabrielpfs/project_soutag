...
# -----------------------------
# FUNCTIONS: CREATE TABLES
# -----------------------------

def create_titles_clean_table():
    """Cria a tabela titles_clean com PRIMARY KEY"""
    query = """
    CREATE TABLE IF NOT EXISTS titles_clean (
    ...
    with engine.connect() as conn:
        try:
            ...
            logger.info("✅ Table 'titles_clean' created with PRIMARY KEY(show_id)")
...
def save_clean_table(df, table_name="titles_clean"):
    ...
    """Salva dados na tabela titles_clean sem sobrescrever PK"""
    try:
        df.to_sql(table_name, engine, if_exists='append', index=False, chunksize=chunk_size)
        logger.info(f"✅ {table_name} data inserted in PostgreSQL with {df.shape[0]} records")
    except SQLAlchemyError as e:
...
def create_titles_by_country(df, table_name="titles_by_country"):
    ...
    """
    """Cria tabela normalizada título × país com FK"""
    ...
    CREATE TABLE IF NOT EXISTS {table_name} (
        show_id TEXT,
        country TEXT,
        CONSTRAINT fk_titles_by_country_show
        FOREIGN KEY (show_id)
    ...
    try:
        countries.to_sql(table_name, engine, if_exists='append', index=False, chunksize=chunk_size)
...
def create_titles_by_genre(df, table_name="titles_by_genre"):
    ...
    """
    """Cria tabela normalizada título × gênero com FK"""
    ...
    CREATE TABLE IF NOT EXISTS {table_name} (
        show_id TEXT,
        genre TEXT,
        CONSTRAINT fk_titles_by_genre_show
        FOREIGN KEY (show_id)
        REFERENCES titles_clean(show_id)
        ON DELETE CASCADE
    );
    ...
    try:
        genres.to_sql(table_name, engine, if_exists='append', index=False, chunksize=chunk_size)
...
def run_transform():
    ...
    # 2.1️⃣ Criar tabela titles_clean com PK
    create_titles_clean_table()

    # 2.2️⃣ Inserir dados sem sobrescrever
    save_clean_table(df_clean)

    # 3️⃣ Normalizações com FK
